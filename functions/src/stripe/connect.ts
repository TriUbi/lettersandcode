...
export const createAuthorAccount = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error al crear la cuenta de autor"
      );
    }
  }
);
...
export const createAuthorAccountLink = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        "internal",
        "Error al crear el link de configuración"
      );
    }
  }
);
